...
TIMINGS_COMPILE= re.compile('WRITING COORDINATES TO RESTART FILE AT STEP')
  ...
  for filename in output_filenames:
    ...
    for line in open(filename,'r'):
      if re.match(TIMINGS_COMPILE,line):
        ...
        #time = re.findall('\d',line_split[3])[0]
        #print time
        #time=float(time.strip())
        #steps=re.findall('\d+',line_split[1])[0]
        steps=int(line_split[7])
  
        #print "steps=" , steps
    #total_time= total_time+time
    total_steps= total_steps+steps
    #print filename, "total steps" , total_steps
